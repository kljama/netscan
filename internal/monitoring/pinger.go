...
type PingFunc func(device state.Device, timeout time.Duration, writer PingWriter, stateMgr StateManager, inFlightCounter *atomic.Int64, totalPingsSent *atomic.Uint64)

// StartPinger runs continuous ICMP monitoring for a single device
func StartPinger(ctx context.Context, wg *sync.WaitGroup, device state.Device, interval time.Duration, timeout time.Duration, writer PingWriter, stateMgr StateManager, limiter *rate.Limiter, inFlightCounter *atomic.Int64, totalPingsSent *atomic.Uint64, pingOp PingFunc) {
	...
	for {
		...
		case <-ctx.Done():
			...
			return
		case <-timer.C:
			// 1. Acquire token from rate limiter (blocks until available or context cancelled)
			if err := limiter.Wait(ctx); err != nil {
				// Context was cancelled while waiting for token
				return
			}

			// 2. Perform the ping operation with in-flight tracking
			pingOp(device, timeout, writer, stateMgr, inFlightCounter, totalPingsSent)

			// 3. Reset timer to schedule next ping after interval
...
// performPing executes a single ping operation
func performPing(device state.Device, timeout time.Duration, writer PingWriter, stateMgr StateManager, inFlightCounter *atomic.Int64, totalPingsSent *atomic.Uint64) {
